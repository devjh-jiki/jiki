...
- **Prefer primary and authoritative sources.** Official docs, standards bodies, government/open-data portals, the project's own source code, and peer-reviewed work rank above blog posts and aggregators. When you cite a secondary source, try to trace it to its primary.
...
- **Date everything that can go stale.** Prices, versions, "current" leaders, and statistics get an as-of date so the reader knows the shelf life.
...
- **No em-dash (`—`, U+2014) in the delivered document.** It renders as an awkward separator and reads as machine-generated. Never use it in headings, table cells, list items, or prose. Rewrite by intent: a `Title — Subtitle` heading becomes `Title: Subtitle`; an inline aside becomes a comma, parentheses, or a split sentence; a `[link](url) — description` list item becomes `[link](url): description`; a source tag becomes `(Source, date)`; an empty/"n/a" table cell becomes a plain hyphen `-`. This applies inside Mermaid node labels too (`["A — B"]` → `["A: B"]`), where the em-dash also risks breaking the parse. En-dash `–`, hyphen `-`, and tilde `~` are fine; only the em-dash is banned.
