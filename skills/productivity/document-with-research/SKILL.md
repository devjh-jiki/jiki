...
description: Produce high-quality Markdown documentation — researching open data and sources on the web, verifying claims, designing the document's structure, writing it, and refining existing material for readability, information flow, and cross-document linking. Use whenever the user wants to document a side project, write or improve a README/guide/reference, organize scattered notes into a coherent doc, research a topic and turn the findings into a structured writeup, fact-check and add citations, or generally "make this doc better / clearer / better organized." Trigger even when the user doesn't say the word "documentation" — e.g. "정리해줘", "이거 문서로 만들어줘", "리드미 좀 다듬어줘", "이 주제 조사해서 정리해줘", "자료 찾아서 표로 만들어줘", "write this up", "turn my notes into docs", "research X and summarize." This is for reference/explanatory docs whose goal is clarity and accuracy. It is NOT for a personal blog post or retrospective whose goal is storytelling (use write-blog-post), marketing/promotional copy meant to convert readers (use marketing-copy), or compacting the current conversation into a handoff for another agent (use handoff).
---
