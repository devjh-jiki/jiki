...
9. Do not force open-source reverse-engineering lenses such as C4 or interface contracts into book study. Book study is primarily about extracting reusable judgment. When the user's actual goal is to *learn a codebase or open-source project by reading its code* — not to study prose — that's `open-source-reverse-engineering-coach`'s job, not this one; say so and redirect rather than coaching a book session over source code. This skill is for reading *writing* (books, essays, docs, articles); the Transfer Lens below points reading toward code, but a session whose center of gravity is a repo belongs to the other coach.
